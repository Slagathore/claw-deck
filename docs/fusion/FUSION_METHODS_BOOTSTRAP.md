...
- [x] Every cross-phase handoff passes artifacts by reference + asserts length/hash. (Artifacts thread in-process (no inline-truncation path) AND, when a `runDir` is set, persist per-phase via `artifactStore`; QA/judge enforce the `REVIEWING:<sha12>` echo via `askVerified`.)
- [x] Reviewers/judges emit the `REVIEWING:` echo and the orchestrator asserts it. (`askVerified` requires the header, asserts `echoMatches`, and re-feeds the full artifact ONCE on mismatch as a logged plumbing warning — `tests/fusion.methods.test.ts` "REVIEWING echo".)
...
- [x] `assay`/`prospect` query Atlas first and fall back when `.fusion/atlas.db` is absent. (IPC wires `atlasQuery` via `locate()` over the code-brain DB when it exists; otherwise the engine logs a WARN and falls back to its doc/grep walk.)
- [x] Chaining (`assay`/`prospect`/`foundry-design` -> `foundry`) passes a seed without re-ingest. (`runMethod` accepts a `seed { contract, artifacts, focus }`, pre-loads it, and skips the `ingest` step; the **→ Foundry (seeded)** button on the method-result panel launches Foundry with that seed. `tests/fusion.methods.test.ts` "§5 chaining".)
- [x] `CHANGE_PLAN.md` + `changes.diff` produced in the worktree. (The `build` step delegates to an editing actor in a worktree and asks it to write `CHANGE_PLAN.md` first; the executor captures the worktree diff.)
