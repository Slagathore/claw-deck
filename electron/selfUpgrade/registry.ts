...
import { generateProposal, localOllamaBackend, openaiCompatBackend, ReflectBackend, buildFacts } from './reflector';
...
function backendFor(s: ReflectSettings): ReflectBackend {
  switch (s.backend) {
    ...
    case 'openclaw':
      ...
      // eslint-disable-next-line @typescript-eslint/no-fallthrough -- exhaustiveness
    case 'local':
    default:
...
export function registerSelfUpgradeHandlers() {
  ...
  ipcMain.handle('selfUpgrade:reflect', async (_e, s: ReflectSettings) => {
    ...
    try {
      const proposal = await generateProposal(backend, ensured.path, s.goal || 'propose a small, safe improvement');
      if (!proposal) return { ok: false, reason: 'model returned no parseable patch set' };
      const v = validatePatchSet(proposal, ensured.path);
      if (!v.ok) return { ok: false, reason: `invalid patch: ${v.reason}`, proposal };
  ...
  ipcMain.handle('selfUpgrade:parseManualPatch', async (_e, text: string) => {
    ...
    if (!proposal) return { ok: false, reason: 'could not parse JSON patch set' };
    const ensured = await ensureSourceTree();
    if (!ensured.ready) return { ok: false, reason: ensured.reason };
    const v = validatePatchSet(proposal, ensured.path);
  ...
  ipcMain.handle('selfUpgrade:run', async (_e, opts: {
    patch: PatchSet;
    autoApply?: boolean;
    ...
    if (result.snapshot) lastSnapshots.set(result.snapshot.id, result.snapshot);
    return result;
  });
  ...
  ipcMain.handle('selfUpgrade:snapshot', async (_e, opts: { label?: string }) => {
    const ensured = await ensureSourceTree();
    if (!ensured.ready) return { ok: false, reason: ensured.reason };
    try {
      const snap = await createSnapshot(ensured.path, opts?.label || 'manual snapshot');
  ...
  ipcMain.handle('selfUpgrade:setOrigin', async (_e, opts: { url: string }) => {
    ...
    if (!opts?.url) {
      const r = await dialog.showMessageBox({
        ...
        detail: 'Create a private repo named "claw-deck" on github.com and paste its SSH/HTTPS URL into Settings → Self-Upgrade.',
        buttons: ['OK']
      });
      return { ok: false, reason: 'user prompt acknowledged', acknowledged: r.response === 0 };
    }
