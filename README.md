...
| Library | One-click Ollama model pulls, a catalog of **real MCP servers** (verified npm `npx` + PyPI `uvx` packages — filesystem, git, fetch, github, playwright, context7, …) with **Install & scan** (`npm pack` → static security scan before you trust it), and system-tool installers (winget/choco) |
